...
val generatedJvmLibsDirs =
    listOf(
        ...
        generatedResourcesDir.resolve("jvmMain/libs/aarch64-unknown-linux-gnu"),
        generatedResourcesDir.resolve("jvmMain/libs/x86_64-apple-darwin"),
        generatedResourcesDir.resolve("jvmMain/libs/x86_64-unknown-linux-gnu")
    )
    ...
    doLast {
        targetDirs.forEach { (target, architectures) ->
            architectures.forEach { archDir ->
                val outputDir = layout.buildDirectory.dir("generatedResources/$target/libs/$archDir")
                copy {
